...
ai.url = ai.urls.shift();
  ...
  updateStyles() {
    ...
    // Ensure collapse mode style is updated if config changes
    CollapseMode.updateStyle();
  },
};
...
class Config {
  ...
  constructor({data: c, save}) {
    ...
    if (Object.keys(cfg || {}).some(k => /^ui|^(css|globalStatus|collapseMode)/.test(k) && cfg[k] !== c[k])) {
        App.globalStyle = '';
        CollapseMode.updateStyle();
    }
    if (!isArray(c.scales))
...
const Events = {
  ...
  onKeyDown(e) {
    ...
    // Global hotkeys (no popup required)
    switch(key) {
        case 'KeyX':
            CollapseMode.toggle();
            dropEvent(e);
...
const Menu = window === top && GM.registerMenuCommand && {
  ...
  register() {
    ...
    Menu.registerAlt();
    // Register the new menu command for the collapse mode
    GM.registerMenuCommand('MPIV: Toggle Image Collapse Mode', CollapseMode.toggle);
  },
  registerAlt() {
...
// New integrated module for the collapse functionality
const CollapseMode = {
    isEnabled: false,
    BODY_CLASS: 'image-collapse-active',

    init() {
        // This function should be called after cfg is loaded
        this.isEnabled = cfg.collapseModeActive;
        this.updateStyle(); // Create the style tag initially
        if (this.isEnabled) {
    ...
    toggle() {
        ...
        document.body.classList.toggle(this.BODY_CLASS, this.isEnabled);
        console.log(`Image collapse mode toggled ${this.isEnabled ? 'ON' : 'OFF'}.`);
    },

    updateStyle() {
        const css = `
            /* The body class acts as a master switch for the effect */
            body.${this.BODY_CLASS} img:not(#${PREFIX}popup) {
                transition: max-width 0.4s ease-in-out, max-height 0.4s ease-in-out;
                max-width: ${cfg.collapseModeSize || '200px'};
                max-height: ${cfg.collapseModeSize || '200px'};
                object-fit: contain;
            }

            body.${this.BODY_CLASS} img:not(#${PREFIX}popup):hover {
                max-width: 100%;
                max-height: 100%;
            }
        `;
        // Use MPIV's utility function to manage the style tag
        Util.addStyle('collapse-mode', css);
    }
};
...
async function setup({rule} = {}) {
  ...
  function closeSetup(event) {
    ...
    if (event && (this.id === '_ok' || isApply)) {
      ...
      Ruler.init();
      App.updateStyles(); // Important to apply new styles from config
      Menu.reRegisterAlt();
      if (isApply)
        ...
        $new('li.options.row', [
          ...
          $new('fragment', [
            ...
            $new('div.flex', {style: 'align-items:center; gap: 0.5em;'}, [
              $newCheck('Collapse Mode*', 'collapseModeActive', 'Enable collapsing all page images. Hotkey: "x"'),
              $new('input#collapseModeSize', {type: 'text', style: 'width: 6em;', placeholder: 'e.g. 200px'}),
            ]),
          ]),
        ]),
        $new('li.options.stretch', [
