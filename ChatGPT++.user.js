...
// @version      1.8
// @description  ChatGPT widescreen mode, prompt buttons, and code copy functionality.
// @author       4ndr0666
...
(function() {
  ...
  function initPromptButtons() {
    const prompts = [
      ...
      {
        title: '❯ Shell',
        prompt: `\`\`\`shell\n\n>>> PASTE TERMINAL OUTPUT HERE <<<\n\n\`\`\`\n\n Proceed to systematically and meticulously integrate all of your recommendations without omitting or breaking any code. This is required to be performed cohesively, dynamically and intelligently. Thoroughly examine all modifications in isolation and validation before integrating them into the final revision. Uphold idempotency and diligently accommodate for all code changes consistently. You are required to respect the established hierarchical order for workflow order and modularity.\n\nProceed to precisely parse your complete, fully-functional, error-free and production ready revision in manageable segments. The code is required to be inclusive of all logic and no more than 4000 lines of code to be accepted and valid. Padding for inflated line numbers is strictly prohibited. Assure correct code formatting by leveraging copy/paste operations. Do this by providing a the complete code in one unique codeblock, appropriatelly formated verbatim and instruct the user to copy the block. Provided your revision is inclusive of all logic, this will avoid any further errors.`
      }
    ];
